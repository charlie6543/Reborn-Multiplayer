...
class PokeBattle_AI
  ...
  PARTNERFUNCTIONS = [0x40, 0x41, 0x55, 0x63, 0x66, 0x67, 0xA0, 0xC1, 0xDF, 0x13a, 0x142, 0x150, 0x151, 0x162, 0x163, 0x164, 0x167, 0x169, 0x170, 0x11d, 0x185, 0x317]
  # Swagger, Flatter, Psych Up, Simple Beam, Entrainment, Skill Swap, Frost Breath, Beat Up, Heal Pulse, Aromatic Mist,
  # Topsy-Turvy, Floral Healing, Gear Up, Flower Shield, Rototiller, Instruct, Pollen Puff, Purify, Spotlight, After You
        ...
        case battler.pbTarget(move)
          ...
          when :AllNonUsers
            ...
            if mondata.trainer && (mondata.trainer.trainertype == :UMBTITANIA || mondata.trainer.trainertype == :UMBAMARIA) && @battle.doublebattle
              ...
              scoremult *= 2
            elsif battler.pbPartner.hp > 0 && (battler.pbPartner.hp.to_f > 0.1 * battler.pbPartner.totalhp || pbAIfaster?(move, nil, battler, battler.pbPartner)) && (!Rejuv || battler.pokemon.bossID != :SPACEAGOOD)
  ...
  def coordinateActions(ai_l, ai_r, op_l, op_r) # changes some scores doesn't choose
    ...
    if PBStuff::STATUSCONDITIONMOVE.include?(bestmove1.move) && PBStuff::STATUSCONDITIONMOVE.include?(bestmove2.move) && moves_1[0][:target].intersect?(moves_2[0][:target])
      nextbest1 = moves_1.find { |scores|
        scores[:moveindex] != moves_1[0][:moveindex] || !scores[:target].intersect?(moves_2[0][:target])
      }
      nextbest2 = moves_2.find { |scores|
        scores[:moveindex] != moves_2[0][:moveindex] || !scores[:target].intersect?(moves_1[0][:target])
      }
      if !nextbest1.nil? && !nextbest2.nil?
        if nextbest1[:score] > nextbest2[:score]
    ...
    if bestmoves_id.all? { |bestmove| PBStuff::CONFUMOVE.include?(bestmove) && ![:CHATTER, :DYNAMICPUNCH].include?(bestmove) } && moves_1[0][:target].intersect?(moves_2[0][:target])
      nextbest1 = moves_1.find { |scores|
        scores[:moveindex] != moves_1[0][:moveindex] || !scores[:target].intersect?(moves_2[0][:target])
      }
      nextbest2 = moves_2.find { |scores|
    ...
    if bestmoves_id.all? { |bestmove| [:ENCORE].include?(bestmove) } && moves_1[0][:target].intersect?(moves_2[0][:target])
      nextbest1 = moves_1.find { |scores|
        scores[:moveindex] != moves_1[0][:moveindex] || !scores[:target].intersect?(moves_2[0][:target])
      }
      nextbest2 = moves_2.find { |scores|
  ...
  def getMoveScore(initialscores = [], scoreindex = -1)
    ...
    if @mondata.skill >= MEDIUMSKILL && !moldBreakerCheck(@attacker, @move) &&
       (
    ...
    if @move.pbIsPriorityMoveAI(@attacker) && @attacker != @opponent.pbPartner
      ...
      # accounting for targetting with psychic terrain priority blocking
      is_prio_blocked = !([:User,:Partner,:UserSide,:OpposingSide,:UserOrPartner,:AllNonUsers].include?(@move.target)) && !([:PERISHSONG, :ROTOTILLER, :FLOWERSHIELD].include?(@move.target))
      score *= 0 if (@battle.FE == :PSYTERRAIN || @battle.state.effects[:PSYTERRAIN] > 0)  && !@opponent.isAirborne? && is_prio_blocked
    ...
    if @move.isSoundBased?
      if (@opponent.ability == :SOUNDPROOF && !moldBreakerCheck(@attacker, @move)) || (@attacker.effects[:ThroatChop] != 0 && !@move.zmove)
        $ai_log_data[@attacker.index].final_score_moves.push(0)
        return 0
      end

      score *= 0.6 if checkAImoves([:THROATCHOP]) && !@move.zmove
    end
    ...
    if @move.powderMove? && (@opponent.hasType?(:GRASS) || @opponent.ability == :OVERCOAT || (@mondata.oppitemworks && @opponent.item == :SAFETYGOGGLES))
      $ai_log_data[@attacker.index].final_score_moves.push(0)
      return 0
    end
    ...
    case @move.function
      ...
      when 0x21 # Charge
        ...
        miniscore *= 1.5 if @attacker.moves.any? { |moveloop| moveloop != nil && moveloop.pbType(@attacker) == :ELECTRIC && moveloop.basedamage > 0 } && @attacker.effects[:Charge] == 0
      ...
      when 0x4e # Captivate
        ...
        if (agender == 2 || ogender == 2 || agender == ogender || @opponent.effects[:Attract] >= 0 || ((@opponent.ability == :OBLIVIOUS || @opponent.ability == :AROMAVEIL || @opponent.pbPartner.ability == :AROMAVEIL) && !moldBreakerCheck(@attacker, @move)))
          miniscore = 0
        else
      ...
      when 0x106 # Grass Pledge
        miniscore *= 1.5 if (@attacker.pbPartner.pbHasMove?(:FIREPLEDGE) && @battle.canChangeFE?(:BURNING)) || (@attacker.pbPartner.pbHasMove?(:WATERPLEDGE) && @battle.canChangeFE?(:SWAMP))
      ...
      when 0x107 # Fire Pledge
        miniscore *= 1.5 if (@attacker.pbPartner.pbHasMove?(:GRASSPLEDGE) && @battle.canChangeFE?(:BURNING)) || (@attacker.pbPartner.pbHasMove?(:WATERPLEDGE) && @battle.canChangeFE?(:RAINBOW))
      ...
      when 0x108 # Water Pledge
        miniscore *= 1.5 if (@attacker.pbPartner.pbHasMove?(:FIREPLEDGE) && @battle.canChangeFE?(:RAINBOW)) || (@attacker.pbPartner.pbHasMove?(:GRASSPLEDGE) && @battle.canChangeFE?(:SWAMP))
      ...
      when 0x138 # Noble Roar, Tearful Look
        statarray = [1, 0, 1, 0, 0, 0, 0]
        statarray = [2, 0, 2, 0, 0, 0, 0] if @mondata.skill >= BESTSKILL && @move.move == :NOBLEROAR && (@battle.FE == :FAIRYTALE || @battle.FE == :DRAGONSDEN)
        miniscore = oppstatdrop(statarray)
      ...
      when 0x160 # Core Enforcer
        if !PBStuff::FIXEDABILITIES.include?(@opponent.ability) && !@opponent.effects[:GastroAcid] && @opponent.effects[:Substitute] <= 0 && (!pbAIfaster?(@move) || checkAIpriority())
  ...
  def sleepcode
    return @move.basedamage > 0 ? 1 : 0 if !(@opponent.pbCanSleep?(false, false, false, moldBreakerCheck(@attacker, @move)) && @opponent.effects[:Yawn] == 0)
  ...
  def poisoncode
    return @move.basedamage > 0 ? 1 : 0 if !@opponent.pbCanPoison?(false, false, @move.move == :TOXIC && @attacker.ability == :CORROSION, moldBreakerCheck(@attacker, @move))
    return @move.basedamage > 0 ? 1 : 0 if hydrationCheck(@opponent)
    return @move.basedamage > 0 ? 1 : 0 if secondaryEffectNegated?()
  ...
  def paracode
    return @move.basedamage > 0 ? 1 : 0 if !@opponent.pbCanParalyze?(false, moldBreakerCheck(@attacker, @move))
    return @move.basedamage > 0 ? 1 : 0 if hydrationCheck(@opponent)
    return @move.basedamage > 0 ? 1 : 0 if secondaryEffectNegated?()
  ...
  def burncode
    return @move.basedamage > 0 ? 1 : 0 if !@opponent.pbCanBurn?(false, false, moldBreakerCheck(@attacker, @move))
    return @move.basedamage > 0 ? 1 : 0 if hydrationCheck(@opponent)
    return @move.basedamage > 0 ? 1 : 0 if secondaryEffectNegated?()
  ...
  def freezecode
    return @move.basedamage > 0 ? 1 : 0 if !@opponent.pbCanFreeze?(false, moldBreakerCheck(@attacker, @move))
    return @move.basedamage > 0 ? 1 : 0 if hydrationCheck(@opponent)
    return @move.basedamage > 0 ? 1 : 0 if secondaryEffectNegated?()
  ...
  def attractcode
    agender = @attacker.gender
    ogender = @opponent.gender
    return 0 if (agender == 2 || ogender == 2 || agender == ogender || @opponent.effects[:Attract] >= 0 || ((@opponent.ability == :OBLIVIOUS || @opponent.ability == :AROMAVEIL || @opponent.pbPartner.ability == :AROMAVEIL) && !moldBreakerCheck(@attacker, @move)))
  ...
  def selfstatboost(stats)
    ...
    if seedProtection?(@attacker) || (@attacker.effects[:Substitute] > 0 || ((@attacker.effects[:Disguise] || (@attacker.effects[:IceFace] && (@opponent.attack > @opponent.spatk || @battle.FE == :FROZENDIMENSION))) && !moldBreakerCheck(@opponent, nil, true)) && !@battle.doublebattle)
      miniscore *= 1.3
    end
    miniscore *= 0.5 if (@opponent.effects[:Substitute] > 0 || ((@opponent.effects[:Disguise] || (@opponent.effects[:IceFace] && (@attacker.attack > @attacker.spatk || @battle.FE == :FROZENDIMENSION))) && !moldBreakerCheck(@attacker, nil, true)))
    ...
    if @mondata.skill >= MEDIUMSKILL
      ...
      if maxdam < (@attacker.hp / 4.0) && sweep
        ...
        miniscore *= 0.1 if maxdam > @attacker.hp && !(@attacker.effects[:Substitute] > 0 || ((@attacker.effects[:Disguise] || (@attacker.effects[:IceFace] && (@opponent.attack > @opponent.spatk || @battle.FE == :FROZENDIMENSION))) && !moldBreakerCheck(@opponent, nil, true)) || seedProtection?(@attacker))
      end
  ...
  def oppstatdrop(stats)
    ...
    if stats[PBStats::DEFENSE] > 0 || stats[PBStats::SPDEF] > 0 # defense stuff
      miniscore *= 1.1
      miniscore *= 1.2 if checkAIdamage() < @attacker.hp
  ...
  def tormentcode
    return 0 if @opponent.effects[:Torment] || ((@opponent.ability == :AROMAVEIL || @opponent.pbPartner.ability == :AROMAVEIL) && !moldBreakerCheck(@attacker, @move))
  ...
  def disablecode
    return 0 if @opponent.effects[:Disable] > 0 || ((@opponent.ability == :AROMAVEIL || @opponent.pbPartner.ability == :AROMAVEIL) && !moldBreakerCheck(@attacker, @move))
  ...
  def tauntcode
    return @move.basedamage > 0 ? 1 : 0 if @opponent.effects[:Taunt] > 0 || ((@opponent.ability == :OBLIVIOUS || @opponent.ability == :AROMAVEIL || @opponent.pbPartner.ability == :AROMAVEIL) && !moldBreakerCheck(@attacker, @move))
  ...
  def encorecode
    return 0 if @opponent.effects[:Encore] > 0 || ((@opponent.ability == :AROMAVEIL || @opponent.pbPartner.ability == :AROMAVEIL) && !moldBreakerCheck(@attacker, @move))
  ...
  def covetcode
    return @move.basedamage > 0 ? 1 : 0 if @opponent.ability == :STICKYHOLD && !moldBreakerCheck(@attacker, @move)
  ...
  def bestowcode
    return 0 if @opponent.ability == :STICKYHOLD && !moldBreakerCheck(@attacker, @move)
  ...
  def subcode
    ...
    miniscore *= 0.3 if getAIMemory().any? { |moveloop| moveloop != nil && moveloop.isSoundBased? && moveloop.basedamage > 0 }
  ...
  def arocode(stat)
    return 0 unless @battle.doublebattle && @opponent.index == @attacker.pbPartner.index && @opponent.stages[stat] != 6
  ...
  def flowershieldcode(score)
    ...
    if @battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN, 2, 5) || @battle.FE == :FAIRYTALE
      score += 20
      miniscore = 100
      miniscore *= 1.3 if @attacker.effects[:Substitute] > 0 || ((@attacker.effects[:Disguise] || (@attacker.effects[:IceFace] && (@opponent.attack > @opponent.spatk || @battle.FE == :FROZENDIMENSION))) && !moldBreakerCheck(@opponent, nil, true))
      miniscore *= 1.3 if @initial_scores.length > 0 && hasbadmoves(20)
  ...
  def flingcode
    ...
    miniscore = 1.0
    case @attacker.item
      when :POISONBARB then miniscore *= 1.2 if @opponent.pbCanPoison?(false, false, @attacker.ability == :CORROSION) && ![:GUTS, :POISONHEAL, :TOXICBOOST, :QUICKFEET, :MARVELSCALE, :MAGICGUARD].include?(@opponent.ability) && @opponent.crested != :ZANGOOSE
      when :TOXICORB
        if @opponent.pbCanPoison?(false, false, @attacker.ability == :CORROSION) && ![:GUTS, :POISONHEAL, :TOXICBOOST, :QUICKFEET, :MAGICGUARD].include?(@opponent.ability) && @opponent.crested != :ZANGOOSE
          miniscore *= 1.2
          miniscore *= 2 if @attacker.pbCanPoison?(false, true, @attacker.ability == :CORROSION) && ![:GUTS, :POISONHEAL, :TOXICBOOST, :QUICKFEET, :MAGICGUARD].include?(@attacker.ability)
        end
      when :FLAMEORB
        if @opponent.pbCanBurn?(false) && ![:GUTS, :FLAREBOOST].include?(@opponent.ability)
          ...
          miniscore *= 2 if @attacker.pbCanBurn?(false, true) && ![:GUTS, :FLAREBOOST, :QUICKFEET, :MARVELSCALE, :MAGICGUARD].include?(@attacker.ability)
        end
  ...
  def pbRoughAccuracy(move, attacker, opponent)
    ...
    evastage = 0 if opponent.effects[:Foresight] || opponent.effects[:MiracleEye] || move.function == 0xA9 || [:UNAWARE, :KEENEYE].include?(attacker.ability)
    accstage -= evastage
    ...
    if @mondata.skill >= MEDIUMSKILL
      ...
      if @mondata.skill >= HIGHSKILL
        ...
        accuracy *= [:BACKALLEY, :CITY].include?(@battle.FE) ? 0.67 : 0.8 if attacker.ability == :HUSTLE && move.basedamage > 0 && move.pbIsPhysical?(move.pbType(attacker))
      end
      if @mondata.skill >= BESTSKILL
        ...
        accuracy *= 0.5 if opponent.ability == :TANGLEDFEET && opponent.effects[:Confusion] > 0 && !moldBreakerCheck(attacker, move)
        accuracy *= 0.8 if (@battle.pbWeather == :SANDSTORM || @battle.FE == :DESERT || @battle.FE == :ASHENBEACH) && opponent.ability == :SANDVEIL && !moldBreakerCheck(attacker, move)
        accuracy *= 0.8 if (@battle.pbWeather == :HAIL || @battle.FE == :ICY || @battle.FE == :SNOWYMOUNTAIN) && opponent.ability == :SNOWCLOAK && !moldBreakerCheck(attacker, move)
      end
  ...
  def pbMoveOrderAI(bestmoves = [nil, nil, nil, nil]) # lol it's just pbPriority
    priorityarray = []
    for i in 0..3
      ...
      priorityarray[i][0] += bestmoves[i].priority if bestmoves[i]
  ...
  def hpGainPerTurn(attacker = @attacker)
    ...
    if attacker.ability != :MAGICGUARD && !(attacker.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
      if (@battle.FE == :BURNING || @battle.FE == :VOLCANIC) && attacker.burningFieldPassiveDamage?
        subscore = 0
  ...
  def pbTypeModNoMessages(type = @move.type, attacker = @attacker, opponent = @opponent, move = @move, skill = @mondata.skill)
    ...
    if !moldBreakerCheck(attacker, move)
      case opponent.ability
    ...
    typemod = move.fieldTypeChange(attacker, opponent, typemod, false)
    typemod = move.overlayTypeChange(attacker, opponent, typemod, false)
    typemod = 0 if opponent.ability == :WONDERGUARD && !moldBreakerCheck(attacker, move) && typemod <= 4
  ...
  def pbChangeMove(move, attacker)
    ...
    case move.move
      ...
      when :NATUREPOWER
        ...
        if move.basedamage > 0 && zmove
          move = PokeBattle_Move.pbFromPBMove(@battle, PBMove.new(PBStuff::CRYSTALTOZMOVE[PBStuff::TYPETOZCRYSTAL[move.type]]), attacker, move)
        end
  ...
  def getSwitchInScoresParty(hard_switch)
    ...
    if @mondata.skill < MEDIUMSKILL
      ...
      # if there are no switching at all
    ...
    for partyindex in 0...party.length
      ...
      pbStatChangingSwitchOpponent(i, @opponent)
      ...
      if @mondata.skill >= HIGHSKILL
        ...
        if i.pbHasMove?(:PURSUIT) || ((i.pbHasMove?(:SANDSTORM) || i.pbHasMove?(:HAIL)) && @opponent.item != :SAFETYGOGGLES)||
           i.pbHasMove?(:TOXIC) || i.pbHasMove?(:LEECHSEED) || (i.pbHasMove?(:CURSE) && i.hasType?(:GHOST))
      ...
      if @mondata.skill >= HIGHSKILL
        case i.ability
          ...
          when :WONDERGUARD
            ...
            dievar = true if moldBreakerCheck(@opponent, nil, true)
            dievar = true if moldBreakerCheck(@opponent.pbPartner, nil, true)
      ...
      monscore *= partyacedrop if theseRoles.include?(:ACE) || (i.species == :SPIRITOMB && i.item == :SPIRITCREST) && @mondata.skill >= BESTSKILL
      # Final score
      monscore.floor
    ...
    if survivors.none? { |lives| lives }
      opponent = pbCloneBattler(firstOpponent().index, illusionCheck(firstOpponent(), @battle.battlers[firstOpponent().index]))
      opponent2 = pbCloneBattler(firstOpponent().pbPartner.index, illusionCheck(firstOpponent().pbPartner, @battle.battlers[firstOpponent().pbPartner.index])) if @battle.doublebattle && firstOpponent().pbPartner.hp > 0

      # innards out
      for mon in partycheck.find_all{ |mon| mon.ability == :INNARDSOUT}
        if (@battle.doublebattle && opponent2)
          partyScores[mon.pokemonIndex] += 500 if mon.hp >= opponent2.hp
        end
        partyScores[mon.pokemonIndex] += 500 if mon.hp >= opponent.hp
      end

      if opponent.ability != :MAGICGUARD && opponent.item != :PROTECTIVEPADS && opponent.ability != :LONGREACH
        # aftermath
        aftermathdmg = @battle.FE == :CORROSIVEMIST ? 50 : 25
        for mon in partycheck.find_all{ |mon| mon.ability == :AFTERMATH}
          if aftermathdmg >= (opponent.hp.to_f / opponent.totalhp) * 100
            partyScores[mon.pokemonIndex] += 200 if checkAIbestMove(opponent, mon).contactMove?
          elsif @battle.doublebattle && opponent2
            if (aftermathdmg >= (opponent2.hp.to_f / opponent2.totalhp) * 100)
              partyScores[mon.pokemonIndex] += 200 if (aftermathdmg >= (opponent2.hp.to_f / opponent2.totalhp) * 100) && checkAIbestMove(opponent2, mon).contactMove?
            end
          end
        end
        ...
        for mon in partycheck.find_all{ |mon| mon.ability}
          if helmetdmg >= (opponent.hp.to_f / opponent.totalhp) * 100
            ...
            if (helmetdmg >= (opponent2.hp.to_f / opponent2.totalhp) * 100)
              partyScores[mon.pokemonIndex] += 200 if checkAIbestMove(opponent2, mon).contactMove?
            end
          end
        end
        ...
        for mon in partycheck.find_all{ |mon| mon.ability == :IRONBARBS || mon.ability == :ROUGHSKIN}
          if contactdmg >= (opponent.hp.to_f / opponent.totalhp) * 100
            ...
            if (contactdmg >= (opponent2.hp.to_f / opponent2.totalhp) * 100)
              partyScores[mon.pokemonIndex] += 200 if checkAIbestMove(opponent2, mon).contactMove?
            end
          end
        end
      ...
      # intimidate
      if opponent.attack > opponent.spatk && !([:DEFIANT, :CONTRARY, :CLEARBODY, :WHITESMOKE, :HYPERCUTTER].include?(opponent.ability) || (Gen > 7 && [:SCRAPPY, :OBLIVIOUS, :OWNTEMPO, :INNERFOCUS].include?(opponent.ability)))
        for mon in partycheck.find_all{ |mon| mon.ability == :INTIMIDATE}
          pbStatChangingSwitchOpponent(mon,opponent)
          pbStatChangingSwitchOpponent(mon,opponent2) if @battle.doublebattle && opponent2
          for mon2 in partycheck
            next if mon2.pokemonIndex == mon.pokemonIndex
            incomingdamage = checkAIdamage(mon2, opponent) if @battle.doublebattle && opponent2
            incomingdamage2 = 0
            incomingdamage2 += checkAIdamage(mon2, opponent2) if @battle.doublebattle && opponent2
      ...
      opponent = pbCloneBattler(firstOpponent().index, illusionCheck(firstOpponent(), @battle.battlers[firstOpponent().index]))
      opponent2 = pbCloneBattler(firstOpponent().pbPartner.index, illusionCheck(firstOpponent().pbPartner, @battle.battlers[firstOpponent().pbPartner.index])) if @battle.doublebattle && opponent2

      # check any move that moves before opponent

      # prankster
      for mon in partycheck.find_all{ |mon| mon.ability == :PRANKSTER}
        break if (opponent.hasType?(:DARK) && !@battle.FE == :BEWITCHED) || (@battle.FE == :PSYCHICTERRAIN && !opponent.isAirborne?)
        check = false
        check = true if mon.moves.any? { |moveloop| moveloop != nil && moveloop.basedamage == 0 && (PBStuff::SLEEPMOVE.include?(moveloop) && opponent.pbCanSleep?(false, false, false, moldBreakerCheck(mon, moveloop)))}
        check = true if mon.moves.any? { |moveloop| moveloop != nil && moveloop.basedamage == 0 && (PBStuff::PARAMOVE.include?(moveloop) && opponent.pbCanParalyze?(false, false, false, moldBreakerCheck(mon, moveloop)))} && (pbRoughStat(@opponent, PBStats::SPEED) / 2.0) < @attacker.pbSpeed && @battle.trickroom == 0 && opponent.ability != :QUICKFEET
        check = true if mon.moves.any? { |moveloop| moveloop != nil && moveloop.basedamage == 0 && (PBStuff::BURNMOVE.include?(moveloop) && opponent.pbCanBurn?(false, false, false, moldBreakerCheck(mon, moveloop)))} && opponent.attack > opponent.spatk && opponent.ability != :GUTS
      ...
      for mon in partycheck
        ...
        for move in mon.moves
          ...
          next if move.priority < checkAIbestMove(opponent, mon).priority || ((move.priority == checkAIbestMove(opponent, mon).priority) && !pbAIfaster?(nil, nil, mon, opponent))
  ...
  def pbStatChangingSwitchOpponent(mon, opponent)
    return if mon.item == :CLEARAMULET && mon.itemWorks?
    if mon.ability == :INTIMIDATE
      case opponent.ability
      when :DEFIANT, :CONTRARY
        opponent.stages[PBStats::ATTACK] += 1
      when :COMPETITIVE
        opponent.stages[PBStats::ATTACK] -= 1
        opponent.stages[PBStats::SPATK] += 2
      when :SCRAPPY, :INNERFOCUS, :OBLIVIOUS, :OWNTEMPO
        opponent.stages[PBStats::ATTACK] -= 1 if Gen <= 7
      when :CLEARBODY, :WHITESMOKE, :FULLMETALBODY, :HYPERCUTTER
      when :RATTLED
        opponent.stages[PBStats::ATTACK] -= 1
        opponent.stages[PBStats::SPEED] += 1 if Gen > 7
      else
        opponent.stages[PBStats::ATTACK] -= 1
      end
    end
    if mon.ability == :PRESSURE && [:DIMENSIONAL, :FROZENDIMENSION].include?(@battle.FE)
      case opponent.ability
      when :CONTRARY
        opponent.stages[PBStats::DEFENSE] += 1
      ...
      when :COMPETITIVE
        ...
        opponent.stages[PBStats::SPDEF] -= 1
        opponent.stages[PBStats::SPATK] += 2
      when :CLEARBODY, :WHITESMOKE, :FULLMETALBODY, :BIGPECKS
      else
        opponent.stages[PBStats::DEFENSE] -= 1
        opponent.stages[PBStats::SPDEF] -= 1
      end
    end
    if mon.ability == :UNNERVE && [:DIMENSIONAL, :FROZENDIMENSION].include?(@battle.FE)
      case opponent.ability
      when :CONTRARY
  ...
  def getAIMemory(battler = @opponent, inspecting = false)
    ...
    if (@mondata.index == battler.index || @mondata.index == battler.pbPartner.index) && battler.is_a?(PokeBattle_Battler) && inspecting != true
      ...
      return ret.find_all { |moveloop| moveloop && moveloop.move }
  ...
  def pbRoughDamage(move = @move, attacker = @attacker, opponent = @opponent)
    ...
    return 0 if opponent.totalhp == 1 && (opponent.ability == :STURDY || (opponent.ability == :STALWART && @battle.FE == :COLOSSEUM)) && move.pbNumHits(attacker) == 1 && !attacker.effects[:ParentalBond] && !attacker.effects[:TyphBond] && !move.pbIsMultiHit && !moldBreakerCheck(attacker, move)

    if @mondata.skill >= MEDIUMSKILL
      basedamage = pbBetterBaseDamage(move, attacker, opponent)
    end
    return 0 if move.zmove && (opponent.effects[:Disguise] || (opponent.effects[:IceFace] && (move.pbIsPhysical? || @battle.FE == :FROZENDIMENSION))) && !moldBreakerCheck(opponent, move)
    ...
    if @mondata.skill >= MEDIUMSKILL
      ...
      if opponent.ability == :MARVELSCALE
      ...
      elsif opponent.ability == :FLUFFY && !moldBreakerCheck(attacker, move)
        ...
        defense = (defense * 2).round if move.pbIsPhysical?(type) && !moldBreakerCheck(attacker, move)
      elsif opponent.ability == :ICESCALES
        defense = (defense * 2).round if move.pbIsSpecial?(type) && !moldBreakerCheck(attacker, move)
    ...
    if @mondata.skill >= MEDIUMSKILL
      if opponent.ability == :MULTISCALE && !moldBreakerCheck(attacker, move) || opponent.ability == :SHADOWSHIELD
    ...
    if opponent.effects[:Substitute] > 0 && attacker.ability != :INFILTRATOR && !move.isSoundBased? &&
       move.move != :SPECTRALTHIEF && move.move != :HYPERSPACEHOLE && move.move != :HYPERSPACEFURY && damage > (opponent.totalhp / 4.0).round
      damage = (opponent.totalhp / 4.0).round
    end
  ...
  def pbBetterBaseDamage(move = @move, attacker = @attacker, opponent = @opponent)
    ...
    case move.function
      ...
      when 0xC1 # Beat Up
        ...
        return basedamage
      when 0xC4 # SolarBeam
        return (basedamage * 0.5).floor if @battle.pbWeather != 0 && @battle.pbWeather != :SUNNYDAY && @battle.pbWeather != :STRONGWINDS
      when 0xD0 # Whirlpool
        if @mondata.skill >= MEDIUMSKILL
          return basedamage * 2 if !$cache.moves[opponent.effects[:TwoTurnAttack]].nil? && $cache.moves[opponent.effects[:TwoTurnAttack]].function == 0xCB # Dive
        end
  ...
  def moldBreakerCheck(battler, move = nil, memory = false)
    return true if [:MOLDBREAKER, :TERAVOLT, :TURBOBLAZE].include?(battler.ability)
    ...
    return true if memory && checkAImoves([:SUNSTEELSTRIKE, :MOONGEISTBEAM, :PHOTONGEYSER, :DECIMATION], battler: battler)
    return false
  end
  ...
  def notOHKO?(attacker, opponent, immediate = false)
    ...
    return true  if attacker.ability == :STURDY && !moldBreakerCheck(opponent, nil, true)
    return true  if Rejuv && attacker.crested == :RAMPARDOS && attacker.pokemon.rampCrestUsed == false
    return true  if @battle.FE == :COLOSSEUM && attacker.ability == :STALWART && !moldBreakerCheck(opponent, nil, true)
