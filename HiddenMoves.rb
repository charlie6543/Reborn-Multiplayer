...
HiddenMoveHandlers::CanUseMove.add(:TELEPORT, lambda { |move, pkmn|
  ...
  if healing
    ...
    if Kernel.pbConfirmMessage(_INTL("Want to return to the last visited healing spot in {1}?", mapname))
      return true
    end
...
def smartTravel()
  ...
  if Kernel.ignoresBadgeRequirements || (HIDDENMOVESCOUNTBADGES ? $Trainer.numbadges >= BADGEFORFLASH : $Trainer.badges[BADGEFORFLASH])
    pkmn = findTMXUser(:FLASH)
    if pkmn && $cache.mapdata[$game_map.map_id].DarkMap && !$PokemonGlobal.flashUsed && !($game_switches[:EasyHMs_Password] && $PokemonBag.pbQuantity(:TM70) > 0)
      Kernel.pbUseHiddenMove(pkmn, :FLASH)
      return
    end
  end
  ...
  if Kernel.ignoresBadgeRequirements || (HIDDENMOVESCOUNTBADGES ? $Trainer.numbadges >= BADGEFORFLY : $Trainer.badges[BADGEFORFLY])
    pkmn = findTMXUser(:FLY)
    if pkmn && !$game_switches[:NoFlyZone] && $cache.mapdata[$game_map.map_id].Outdoor && !$game_switches[:Riding_Tauros]
      ...
      if ret
        ...
        Kernel.pbUseHiddenMove(pkmn, :FLY)
      end
  ...
  pkmn = findTMXUser(:DIG)
  escape = ($PokemonGlobal.escapePoint rescue nil)
  if escape && escape != []
    mapname = pbGetMapNameFromId(escape[0])

    if pkmn
      if Kernel.pbConfirmMessage(_INTL("Use Dig to return to {1}?", mapname))
        Kernel.pbUseHiddenMove(pkmn, :DIG)
      end
      return
    end

    if $PokemonBag.pbQuantity(:ESCAPEROPE) > 0
      if Kernel.pbConfirmMessage(_INTL("Use Escape Rope to return to {1}?", mapname))
  ...
  pkmn = findTMXUser(:TELEPORT)
  healing = $PokemonGlobal.healingSpot
  healing = $cache.metadata[:home] if !healing
  if pkmn && healing && $cache.mapdata[$game_map.map_id].Outdoor
    mapname = pbGetMapNameFromId(healing[0])
    if Kernel.pbConfirmMessage(_INTL("Use Teleport to return to {1}?", mapname))
      Kernel.pbUseHiddenMove(pkmn, :TELEPORT)
    end
    return
  end
...
def findTMXUser(move)
  tm = getTMFromMove(move)
  $Trainer.pokemonParty.each do |pkmn|
    return pkmn if $game_switches[:EasyHMs_Password] && (tm.nil? || $PokemonBag.pbQuantity(tm.item) > 0)
    return pkmn if pkmn.knowsMove?(move)
  end
  return nil
end
