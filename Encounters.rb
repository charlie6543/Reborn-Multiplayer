...
class PokemonEncounters
  ...
  def isCosmeticForm(species, form)
    ...
    baseform = form == 0 ? 1 : 0
    return false if $cache.pkmn[species, baseform].Type1 != $cache.pkmn[species, form].Type1
    return false if $cache.pkmn[species, baseform].Type2 != $cache.pkmn[species, form].Type2
    return false if $cache.pkmn[species, baseform].BaseStats != $cache.pkmn[species, form].BaseStats
    return false if $cache.pkmn[species, baseform].Abilities != $cache.pkmn[species, form].Abilities
    return false if $cache.pkmn[species, baseform].HiddenAbility != $cache.pkmn[species, form].HiddenAbility
    return true
  end
