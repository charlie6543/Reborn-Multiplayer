module PBExp
  ...
  # exceed the maximum Exp. Points for the given growth rate, also
  # prevents negative exp from going lower than intended
  ...
  def PBExp.addExperience(currexp, expgain, growth)
    ...
    exp = maxexp if exp > maxexp && (posExp? || noExp?) # Positive/Default EXP Flow
    exp = 0 if (currexp + expgain < 0) && negExp? # Negative EXP Flow
    return exp
  end
