...
def pbLoadPokemonBitmapSpecies(pokemon, species, back = false)
  ...
  x = 0 if pokemon.isEgg? && $game_switches && $game_switches[:BasicEggs_Password]
  y = form * 384
  ...
  if pokemon.isEgg?
    ...
    bitmapFileName = sprintf("Graphics/Battlers/Egg") if !pbResolveBitmap(bitmapFileName) || ($game_switches && $game_switches[:BasicEggs_Password])
    bitmapFileName = pbResolveBitmap(bitmapFileName)
  else
...
def pbPokemonIconBitmap(pokemon, egg = false) # pbpokemonbitmap, but for icons
  begin
    ...
    filename = sprintf("Graphics/Icons/icon%03d%s%s", species, girl, eggtag)
    filename = sprintf("Graphics/Icons/icon%03d%s", species, eggtag) if !pbResolveBitmap(filename)
    filename = sprintf("Graphics/Icons/%s%s%s", name, girl, eggtag) if !pbResolveBitmap(filename)
    filename = sprintf("Graphics/Icons/%s%s", name, eggtag) if !pbResolveBitmap(filename)
    filename = sprintf("Graphics/Icons/iconEgg") if !pbResolveBitmap(filename) || (egg && $game_switches && $game_switches[:BasicEggs_Password])
  ...
  x = shiny ? 128 : 0
  x = 0 if egg && $game_switches && $game_switches[:BasicEggs_Password]
  y = form * 64
...
def pbIconBitmap(species, form = 0, shiny = false, girl = false, egg = false) # pbpokemonbitmap, but for icons
  ...
  filename = sprintf("Graphics/Icons/icon%03d%s%s", species, girl, eggtag) if !pbResolveBitmap(filename)
  filename = sprintf("Graphics/Icons/icon%03d%s", species, eggtag) if !pbResolveBitmap(filename)
  filename = sprintf("Graphics/Icons/iconEgg") if !pbResolveBitmap(filename) || (egg && $game_switches && $game_switches[:BasicEggs_Password])
  filename = sprintf("Graphics/Icons/icon000") if !pbResolveBitmap(filename)
  iconbitmap = RPG::Cache.load_bitmap(filename)
...
def pbMailBackFile(item)
  ...
  return sprintf("Graphics/Pictures/mail%s", item)
end
...
def LevelLimitExpGain(pokemon, exp) # For exp candies
  ...
  if pokemon.level >= LEVELCAPS[leadersDefeated] || pokemon.level >= 100 + $game_variables[:Extended_Max_Level] || noExp? || negExp?
    return -1
  ...
  pbFadeOutIn(99999) {
    ...
    loop do
      ...
      if chosen >= 0
        ...
        elsif Rejuv && $Trainer.tutorlist.length > 0 && $Trainer.tutorlist.include?(move) && bytutor == false
          gear = Rejuv ? "Cybernav" : "Pokégear"
