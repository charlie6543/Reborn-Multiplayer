...
class PokemonBag_Scene
  ...
  def pbChooseItem
    ...
    pbActivateWindow(@sprites, "itemwindow") {
      ...
      loop do
        ...
        if Input.trigger?(Input::X) || Input.trigger?(Input::A)
          pbHandleSort(itemwindow.pocket, Input.trigger?(Input::X) ? :name : :type)
          pbRefresh
        end
  ...
  def pbHandleSort(pocket, sortBy)
    items = @bag.getPocketItems(pocket)
    pbSortByName(items) if sortBy == :name && pocket != TMPOCKET
    pbSortByMoveName(items) if sortBy == :name && pocket == TMPOCKET
    pbSortByItemType(items) if sortBy == :type
  end

  def pbSortByName(items)
    result = items.sort { |a, b| getItemName(a) <=> getItemName(b) }
