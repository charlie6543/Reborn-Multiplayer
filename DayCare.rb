...
def transferEggMoves(gifter, recipient, recIndex)
  if recipient.moves.length < 4
    ...
    baby = pbGetBabySpecies(gifter.species, gifter.form)
    baby = pbGetNonIncenseLowestSpecies(baby[0], baby[1])

    eggmoves = $cache.pkmn[baby[0], baby[1]].EggMoves
  ...
  for i in 0...2
    ...
    if (pkmn.exp < maxexp && posExp?) || (pkmn.exp > 0 && negExp?)
      ...
      if $game_switches[:Percent_Exp_Gains]
        ...
        pkmn.exp = 0 if (pkmn.exp + ($game_variables[:Exp_DayCare] / 100).floor) < 0
