  ...
  def pbEffectsOnDealingDamage(move, user, target, damage, innards)
    ...
    if target.damagestate.calcdamage > 0 && !target.damagestate.substitute
      if target.ability != :SHIELDDUST || target.moldbroken
        if (user.hasWorkingItem(:KINGSROCK) || user.hasWorkingItem(:RAZORFANG)) && !move.canFlinch? && !(user.ability == :SHEERFORCE && basemove.effect > 0)
  ...
  def pbProcessMoveAgainstTarget(basemove, user, target, numhits, flags = { totaldamage: 0 }, nocheck = false, alltargets = nil, showanimation = true, precheckedacc: nil)
    ...
    for i in 0...numhits
      ...
      if !nocheck && !pbSuccessCheck(basemove, user, target, flags, i == 0 || basemove.function == 0xBF, precheckedacc: precheckedacc) # Triple Kick
        ...
        if basemove.function == 0x175 && (!user.midwayThroughMove || basemove.loopcount == alltargets.length) && i == 0 && user.ability != :MAGICGUARD && !(user.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
      ...
      if user.hasWorkingItem(:EJECTPACK) && user.statLowered
        if !user.isFainted? && @battle.pbCanChooseNonActive?(user.index) && !@battle.pbAllFainted?(@battle.pbParty(user.index))
          ...
          if @battle.FE == :COLOSSEUM
            ...
            @battle.pbClearChoices(user.index)
