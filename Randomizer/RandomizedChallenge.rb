...
KEEPING FOR TYPE STUFF! DO NOT LOAD!
...
=end
  ...
  def buildRandomizer
    ...
    Dir.mkdir("RandData") unless Dir.exist?("RandData")
    Dir.mkdir("RandData/#{@settings.name}") unless Dir.exist?("RandData/#{@settings.name}")
...
class PokeBattle_Pokemon
  ...
  def initialize(species, level, player = nil, withMoves = true, form = 0, fromTrainer = false)
    ...
    if fromTrainer
      ...
      self.__rc_initialize(species, level, player, withMoves, form)
...
def pbLoadTrainer(trainerid, trainername, partyid = 0)
  begin
    ...
    for poke in trainer[0]
      ...
      level = poke[TPLEVEL]
      pokemon = PokeBattle_Pokemon.new(species, level, opponent, true, 0, true) # added a flag to specify from a trainer'
