...
def pbSpriteSetAnimFrame(sprite, frame, user = nil, target = nil, ineditor = false)
  ...
  sprite.visible = true
  if !target.nil? && @target && @target.is_a?(PokeBattle_Battler) && target.index == @target.index &&
     ((@target.fainted && !(Rejuv && @target.isbossmon && @target.shieldCount > 0)) || (@movedata && @movedata.target == :OpposingSide && @target.vanished))
...
class PBAnimations < Array
  ...
  def save(index)
    ...
    name = sprintf("Anim%04d%s.rb", index, (animation.name != "" ? " - " + animation.name.sub(":", " ").gsub(/[^0-9A-Za-z ]/, "") : ""))
    Dir.glob(sprintf("Animations/Anim%04d*.rb", index)).each { |file| File.delete(file) }
    File.write("Animations/" + name, (AnimationDumper.new).dump(animation))
  end
...
class PBAnimationPlayerX
  ...
  def initialize(animation, user, target, scene = nil, move = nil, oppmove = false, ineditor = false)
    ...
    @alloppopp = true if @movedata && @movedata.target == :AllOpposing && animation.name[/^OppZ?Move\:\s*(.*)$/]
    ...
    @realtarget = animation.name[/^OppZ?Move\:\s*(.*)$/] ? user : target
    @realuser = animation.name[/^OppZ?Move\:\s*(.*)$/] ? target : user
    ...
    @isOppMove = animation.name[/^OppZ?Move\:\s*(.*)$/] ? true : false
          ...
          when 3   # Focused on user and target
            if @srcLine && @dstLine
              if @partnerbs && cel[AnimFrame::PATTERN] != -2 && cel[AnimFrame::PATTERN] != -1
                if @alloppopp
                  point = transformPoint(
                    @srcLine[2], @srcLine[3], @srcLine[0], @srcLine[1],
...
class AnimationDumper
  ...
  def dump(animation, level = 0)
    ...
    animation.array.each_with_index do |frame, i|
      ...
      frame.each_with_index do |cel, j|
        ...
        if cel.length == 27 # Compatibility with newer essentials
          if cel[6] != 1
            puts sprintf('WARNING: Animation "%s", frame %d, cel %d is using newer essentials format with a VISIBILITY (%s) which we don\'t support.', animation.name, i, j, cel[6])
          end
          if ([cel[21], cel[22], cel[23], cel[24]] - [0, nil]).length > 0
            puts sprintf('WARNING: Animation "%s", frame %d, cel %d is using newer essentials format with a FLASH color (%s,%s,%s,%s) which we don\'t support.', animation.name, i, j, cel[21], cel[22], cel[23], cel[24])
          end
          cel = [
            cel[0],
            cel[1],
            ...
            cel[4],
            cel[5],
            # 6 = VISIBLE in newer essentials, we don't have that
            cel[7],
            cel[8],
            # 10 and 11 seem to be unused in newer essentials
            cel[11],
            cel[12],
            cel[13],
            cel[14],
            ...
            cel[19],
            cel[20],
            # 21-24 = FLASH* properties in newer essentials, we don't have those
            cel[25],
            cel[26],
          ]
        end
  ...
  def timing(timing, level)
    ...
    value += "flash_color: " + color(timing.flashColor) + ", " if timing.flashColor != AnimationFactory::WHITE && timing.flashColor != nil
