...
class PokemonNestMapScene
  def pbStartScene(species, regionmap = -1)
    ...
    for mapid in 1...$cache.mapdata.length
      next unless pbFindEncounter(mapid, species)
      if $cache.mapdata[mapid].Switches
        ignore = false
        $cache.mapdata[mapid].Switches.each do |switch|
          ignore = true if $game_switches[switch[0]] != switch[1]
        end
        next if ignore
      end
