...
class PokeBattle_Scene
  ...
  def ttsMove(battler, moveIndex, zmove)
    ...
    if pbFieldNotesBattle(battler.moves[moveIndex]) == 1
      fieldboostphrase = ", field-boosted"
    elsif pbFieldNotesBattle(battler.moves[moveIndex]) == 2
      fieldboostphrase = ", field-diminished"
    end
    tts(sprintf("%s, %s%s type%s%s", getMoveName(move.move), move.pbType(battler).name, secondtypephrase, ppphrase, fieldboostphrase))
  end
