...
class PokeBattle_Battle
  ...
  def setField(fieldeffect, temp = 0, add_on: false, growth: false)
    return if @field.effect == fieldeffect && fieldeffect != :RAINBOW
  ...
  def setPledge(moveid, fielduration = 4)
    ...
    setField(Rejuv ? :VOLCANIC : :BURNING, fielduration) if !pledgepair.include?(:WATERPLEDGE)
    @field.pledge = nil
  end
  ...
  def shouldApplyFieldChangeDamageBoost(basemove, user)
    ...
    countersbackup = [@battle.field.counter, @battle.field.counter2, @battle.field.counter3, @battle.field.counter4]

    conditions = @battle.field.fieldChangeData
    if fieldmove && fieldmove[:counterincrease]
      applyCounterIncrease(fieldmove[:counterincrease][0], fieldmove[:counterincrease][1])
    end
    ...
    change = false if @battle.ProgressiveFieldCheck("All") && (PBFields::CONCERT.include?(fieldmove[:fieldchange]) || PBFields::FLOWERGARDEN.include?(fieldmove[:fieldchange]) || PBFields::DARKNESS.include?(fieldmove[:fieldchange]))

    # revert counters, this is not the right time to actually increase them because of spread moves in doubles
    @battle.field.counter, @battle.field.counter2, @battle.field.counter3, @battle.field.counter4 = *countersbackup
    return change
  end

  def applyCounterIncrease(counter, increase)
    case counter
      when 1 then @battle.field.counter += increase
      when 2 then @battle.field.counter2 += increase
      when 3 then @battle.field.counter3 += increase
      when 4 then @battle.field.counter4 += increase
    end
  end
